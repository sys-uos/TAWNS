...
import pytest
from scaper.simulatorCore import EventSpec
import tempfile
...
def test_get_value_from_dist():
    rng = scaper.simulatorUtils._check_random_state(0)
    # const
    x = scaper.simulatorCore._get_value_from_dist(('const', 1), rng)
    assert x == 1

    # choose
    for _ in range(10):
        x = scaper.simulatorCore._get_value_from_dist(('choose', [1, 2, 3]), rng)
        assert x in [1, 2, 3]
    ...
    # normal
    for _ in range(10):
        x = scaper.simulatorCore._get_value_from_dist(('normal', 5, 1), rng)
        assert scaper.simulatorUtils.is_real_number(x)

    # truncnorm
    for _ in range(10):
        x = scaper.simulatorCore._get_value_from_dist(('truncnorm', 5, 10, 0, 10), rng)
        assert scaper.simulatorUtils.is_real_number(x)
        assert 0 <= x <= 10
    ...
    def __test_bad_tuple_list(tuple_list):
        ...
        for t in tuple_list:
            if isinstance(t, tuple):
                ...
                print(t)
            pytest.raises(ScaperError, scaper.simulatorCore._get_value_from_dist, t, random_state=rng)
...
def test_ensure_satisfiable_source_time_tuple():
    ...
    _adjusted, warn = scaper.simulatorCore._ensure_satisfiable_source_time_tuple(
    ...
    def __test_bad_tuple_list(tuple_list):
        for t in tuple_list:
            if isinstance(t, tuple):
                ...
                print(t)
            pytest.raises(ScaperError, scaper.simulatorCore._validate_distribution, t)
...
def test_validate_label():
    ...
    allowed_labels = ['yes']
    pytest.raises(ScaperError, scaper.simulatorCore._validate_label, ('const', 'no'),
                  allowed_labels)

    # Choose list must be subset of allowed labels
    allowed_labels = ['yes', 'hello']
    pytest.raises(ScaperError, scaper.simulatorCore._validate_label, ('choose', ['no']),
                  allowed_labels)
    ...
    for bld in bad_label_dists:
        pytest.raises(ScaperError, scaper.simulatorCore._validate_label, bld,
                      allowed_labels)
...
def test_validate_source_file():
    ...
    with backports.tempfile.TemporaryDirectory() as tmpdir:
        ...
        pytest.raises(ScaperError, scaper.simulatorCore._validate_source_file,
    ...
    bad_label_dists = [('uniform', 0, 1), ('normal', 0, 1),
                       ('truncnorm', 0, 1, 0, 1)]
    for bld in bad_label_dists:
        pytest.raises(ScaperError, scaper.simulatorCore._validate_source_file, bld,
                      ('const', 'siren'))
...
def test_validate_time():
    ...
    def __test_bad_time_tuple(time_tuple):
        pytest.raises(ScaperError, scaper.simulatorCore._validate_time, time_tuple)
    ...
    pytest.warns(ScaperWarning, scaper.simulatorCore._validate_time, ('normal', 5, 2))
...
def test_validate_duration():
    ...
    def __test_bad_duration_tuple(duration_tuple):
        pytest.raises(ScaperError, scaper.simulatorCore._validate_duration,
                      duration_tuple)
    ...
    # using normal will issue a warning since it can generate neg values
    pytest.warns(ScaperWarning, scaper.simulatorCore._validate_duration,
                 ('normal', 5, 2))
...
def test_validate_snr():
    ...
    def __test_bad_snr_tuple(snr_tuple):
        pytest.raises(ScaperError, scaper.simulatorCore._validate_snr, snr_tuple)
...
def test_validate_pitch_shift():
    ...
    def __test_bad_ps_tuple(ps_tuple):
        pytest.raises(ScaperError, scaper.simulatorCore._validate_pitch_shift, ps_tuple)
...
def test_validate_time_stretch():
    ...
    def __test_bad_ts_tuple(ts_tuple):
        pytest.raises(ScaperError, scaper.simulatorCore._validate_time_stretch,
                      ts_tuple)
    ...
    pytest.warns(
        ScaperWarning, scaper.simulatorCore._validate_time_stretch, ('normal', 5, 1))
...
def test_validate_event():
    ...
    for bal in bad_allowed_labels:
        pytest.raises(ScaperError, scaper.simulatorCore._validate_event,
                      label=('choose', []),
                      source_file=('choose', []),
                      source_time=('const', 0),
                      event_time=('const', 0),
...
def test_scaper_instantiate_event():
    ...
    assert scaper.simulatorUtils.is_real_number(instantiated_event.pitch_shift)
...
def test_generate_with_seeding(atol=1e-4, rtol=1e-8):
    ...
    seeds = [
        0, 10, 20, 
        scaper.simulatorUtils._check_random_state(0),
        scaper.simulatorUtils._check_random_state(10),
        scaper.simulatorUtils._check_random_state(20)
    ]
    num_generators = 2
    for seed in seeds:
        generators = []
