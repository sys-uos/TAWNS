...
class NORMALIZATION(str, Enum):
    NONE = "NONE" # todo: not applied yet
    LUFS = "LUFS"
    PEAK = "PEAK" # todo: not applied yet
    RMS = "RMS"
            ...
            for i, e in enumerate(ann.data):
                if e.value['role'] == 'background':
                    ...
                    with _close_temp_files(tmpfiles_internal):
                        ...
                        if normalization == NORMALIZATION.LUFS:
                            # NOW compute LUFS
                        ...
                        elif normalization == NORMALIZATION.RMS:
                            from pydub import AudioSegment
                            ast = AudioSegment.from_file(e.value['source_file'])
                            bg_dbRMS_FS = ast.dBFS  # rms in dBFS
                            del ast

                            # set lower bound to -120 (necessary for silent/empty background audio)
                            gain = self.ref_db - max(bg_dbRMS_FS, -120)
                            event_audio = np.exp(gain * np.log(10) / 20) * event_audio
                    ...
                    with _close_temp_files(tmpfiles_internal):
                        ...
                        if normalization == NORMALIZATION.LUFS:
                            ...
                            from pydub import AudioSegment
                            ast = AudioSegment.from_file(e.value['source_file'])
                            fg_dbRMS_FS = ast.dBFS  # rms in dBFS
                            del ast

                            gain =  self.ref_db - fg_dbRMS_FS + e.value['snr']
                            event_audio = np.exp(gain * np.log(10) / 20) * event_audio
